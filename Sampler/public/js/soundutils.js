  async function loadAndDecodeSound(urls, ctx) {
    const responses = await Promise.all(urls.map((url) => fetch(url)));
    const sounds = await Promise.all(responses.map((response) => response.arrayBuffer()));
    const decodedSounds = await Promise.all(sounds.map((sound) => ctx.decodeAudioData(sound)));
